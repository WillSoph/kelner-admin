    ...
    return (
        <div className=" rounded-lg p-4 flex space-x-3">
            ...
            <div className="flex flex-col space-y-3 flex-grow">
                ...
                <h1 className="font-semibold text-lg text-orange-500">R$ {parseFloat(props.fields.preco?.stringValue).toFixed(2)}</h1>
                <p className="text-sm poppins text-gray-400">{props.fields.categoria?.stringValue}</p>
            </div>

            <div className="flex items-center px-4 py-2 space-x-3">
                <span className="text-lg text-gray-700 poppins select-none">{props.quantity} itens</span>
            </div>
           {/*  quantity 
            <div className="flex items-center px-4 py-2 space-x-3">
                <AiOutlineMinus
            ...
            <div className="flex flex-col items-center justify-center">
                <AiOutlineDelete className="w-6 h-6 text-gray-600 transform transition hover:scale-105 duration-500 cursor-pointer" onClick={() => removeOrder(props.fields.nome?.stringValue)} />
            </div>
        </div>
    )
}
