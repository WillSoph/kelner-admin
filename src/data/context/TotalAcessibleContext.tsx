...
const TotalAcessibleContext = createContext<TotalAcessibleContextType | undefined>(undefined);
...
export function TotalAcessibleProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <TotalAcessibleContext.Provider value={{ totalAcessible, setTotalAcessible }}>
      {children}
...
export function useTotalAcessible() {
  ...
  if (!context) {
    throw new Error('useTotalAcessible must be used within a TotalAcessibleProvider');
  }
  return context;
}